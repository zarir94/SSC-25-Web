...
export const load: PageServerLoad = async ({ params }) => {
    ...
    let kuet_marks = [...results.filter(i=>i.school == 'KUET').map(i=>i.total_marks), result.total_marks].sort((a,b)=>b-a);
    ...
    result.rank_kuet = kuet_marks.indexOf(result.total_marks) + 1;
